...
def fig_iterazioni_m1_m2():
    print('Raccolta dati iterazioni M1 vs M2 su UC1 (K=30) ...')
    nm  = build_noise_model(**NOISE_REALISTIC)
    ...
    # Annota media
    for i, arr in enumerate([m1, m2], start=1):
        ax.text(i, arr.mean() + 0.8, f'$\\bar{{M}}={arr.mean():.2f}$',
                ha='center', fontsize=10, color='black')

    # p-value Mann-Whitney
    stat, p = mannwhitneyu(m1, m2, alternative='greater')
    ax.text(0.98, 0.97, f'Mann-Whitney $p={p:.4f}$',
            transform=ax.transAxes, ha='right', va='top', fontsize=9,
            bbox=dict(boxstyle='round,pad=0.3', facecolor='white', alpha=0.8))

    ax.set_ylabel('Iterazioni per ottenere i fattori corretti', fontsize=10)
    ax.set_title('UC1 — Distribuzione iterazioni: Metodo 1 vs Metodo 2\n'
                 '($N=15$, NISQ-realistico, $K=30$ ripetizioni, $M_{\\mathrm{max}}=50$)', fontsize=11)
    ax.set_ylim(0, 55)
    ax.axhline(50, color='gray', linestyle=':', linewidth=1, alpha=0.6)
    ax.text(2.35, 50.5, '$M_{\\mathrm{max}}$', fontsize=8, color='gray')
