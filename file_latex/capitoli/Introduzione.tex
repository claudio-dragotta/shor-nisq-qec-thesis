...
L'elaborato è organizzato in undici capitoli. Il Capitolo~2 svolge una funzione trasversale di raccordo tra teoria e pratica; i Capitoli~3--6 costituiscono la parte teorica; i Capitoli~7--11 la parte sperimentale.
...
\medskip
\textbf{Parte sperimentale (Capitoli 7--11).}
...
Il \textbf{Capitolo~\ref{chap:risultati}} (\textit{Risultati Sperimentali: dal Classificatore ML alla Strategia TOP-K}) riporta l'intera campagna sperimentale, documentando che cosa è stato testato e perché. Stabilisce la baseline del Metodo 1 sui quattro use case ($\bar{M}_1 = 6.43$ iterazioni per UC1, $\bar{M}_1 = 2.42$ per UC2; UC3 e UC4 inaccessibili per l'elevata profondità di circuito, $>10\,000$ porte CX, prodotta dalla decomposizione \texttt{UnitaryGate}); verifica sperimentalmente il classificatore \ac{ML} e ne isola il contributo tramite analisi di ablazione: il guadagno ($\rho = 6.44$ su UC1, $p = 0.0002$) è interamente attribuibile alla strategia di ricerca multi-candidato TOP-4, mentre il classificatore risulta statisticamente irrilevante su UC1 ($p = 0.849$) e controproducente su UC2 ($\rho = 0.731$ contro $\rho = 2.42$ di $M_{\text{TOP4}}$). Completano il capitolo la campagna parametrica sistematica su $M_{\text{TOP4}}$, il suo posizionamento nel quadro delle strategie di mitigazione e il confronto sperimentale con la Zero-Noise Extrapolation.
