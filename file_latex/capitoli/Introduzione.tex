...
Questo lavoro si propone di affrontare tale sfida integrando due ambiti in rapida evoluzione: il \textbf{calcolo quantistico} e i \textbf{modelli di \ac{ML}}. L'obiettivo centrale è eseguire l'algoritmo di Shor su simulatori quantistici realistici con noise model calibrati sull'hardware IBM, e dimostrare sperimentalmente che un classificatore \ac{ML} pre-addestrato sugli output rumorosi del circuito \ac{QPE} può ridurre il numero medio di esecuzioni necessarie per ottenere i fattori corretti.

Il \textbf{contributo originale} è il confronto sperimentale tra due metodi di estrazione del risultato:
\begin{itemize}
    \item \textbf{Metodo 1} (baseline classico): si ripete l'esecuzione del circuito raccogliendo l'istogramma degli output; il candidato è la misura più frequente (strategia TOP-1). Richiede in media $\bar{M}_1 = 6.43$ esecuzioni per $N=15$ in condizioni NISQ-realistiche.
    \item \textbf{Metodo 2} (classificatore \ac{ML}): un classificatore binario (Random Forest) addestrato su 2\,000 istogrammi sintetici decide, prima di tentare l'estrazione, se il segnale \ac{QPE} nell'istogramma è recuperabile. La ricerca è estesa ai 4 candidati più frequenti (strategia TOP-4). Ottiene $\bar{M}_2 = 1.00$ esecuzione per $N=15$ in condizioni NISQ-realistiche, con fattore di riduzione $\rho = \bar{M}_1/\bar{M}_2 = 6.44$ statisticamente significativo ($p = 0.0002$, Mann-Whitney U).
\end{itemize}

L'elaborato si sviluppa dalle fondamenta teoriche del calcolo quantistico fino ai risultati sperimentali su simulatori rumorosi, fornendo al lettore gli strumenti concettuali e pratici per comprendere sia le potenzialità che i limiti dell'algoritmo di Shor nell'era NISQ, e le prospettive aperte dall'integrazione con il \ac{ML}. Gli obiettivi specifici e la struttura dettagliata del lavoro sono descritti nel Capitolo~\ref{chap:obiettivi}.
...
Il \textbf{Capitolo~\ref{chap:risultati2}} (\textit{Risultati --- Metodo 2}) confronta il Metodo \ac{ML}-assistito con la baseline: per UC1 il fattore di riduzione $\rho = \bar{M}_1/\bar{M}_2 = 6.44$ ($p = 0.0002$, Mann-Whitney U, statisticamente significativo), con $\bar{M}_2 = 1.00$ iterazione. Per UC2 il metodo non produce miglioramenti significativi ($\rho = 0.731$, $p = 0.704$), evidenziando un limite del classificatore in regime ad alto rumore dove il Metodo 1 raggiunge già $p_{\text{iter}} \approx 41\%$.

I \textbf{Capitoli~\ref{chap:conclusioni1}} e~\textbf{\ref{chap:conclusioni2}} (\textit{Conclusioni --- Metodo 1} e \textit{Metodo 2}) sintetizzano il significato dei risultati ottenuti, valutano il raggiungimento degli obiettivi, discutono i limiti e identificano il regime di applicabilità ottimale del Metodo 2.
