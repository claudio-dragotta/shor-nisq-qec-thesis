...
L'elaborato è organizzato in quattordici capitoli, articolati in due parti distinte: una parte teorica che fornisce le fondamenta concettuali necessarie, e una parte sperimentale che presenta il sistema sviluppato e i risultati ottenuti.

\textbf{Parte teorica (Capitoli 2--6).}

Il \textbf{Capitolo~\ref{chap:obiettivi}} (\textit{Obiettivi}) definisce le domande di ricerca e i traguardi concreti del lavoro, introducendo il filo conduttore che attraversa l'intera tesi.

Il \textbf{Capitolo~\ref{chap:fondamenti}} (\textit{Fondamenti del Calcolo Quantistico}) introduce i concetti matematici e fisici indispensabili: il formalismo dei qubit, la sovrapposizione, l'entanglement, le porte logiche quantistiche e il modello a circuiti, con particolare attenzione ai qubit superconduttori della piattaforma IBM Quantum.

Il \textbf{Capitolo~\ref{chap:shor}} (\textit{L'Algoritmo di Shor}) presenta la struttura dell'algoritmo: la riduzione della fattorizzazione al \textit{period finding}, la Quantum Phase Estimation, la complessità polinomiale e le implicazioni crittografiche per RSA e per la transizione verso la crittografia post-quantistica.

Il \textbf{Capitolo~\ref{chap:rumore}} (\textit{Rumore Quantistico nei Sistemi NISQ}) analizza le quattro fonti fisiche di degrado dello stato quantistico --- decoerenza ($T_1$, $T_2$), errori di gate, readout errors e crosstalk --- e le formalizza tramite i quattro canali matematici fondamentali (bit flip, phase flip, depolarizzante, amplitude damping). Il capitolo quantifica l'impatto cumulativo del rumore sull'algoritmo di Shor, mostrando come la probabilità di successo crolli al $3.4\%$ già per $N=15$ su hardware NISQ.

Il \textbf{Capitolo~\ref{chap:strategie}} (\textit{Strategie per la Riduzione del Rumore Quantistico}) descrive e confronta le quattro macro-strategie di mitigazione del rumore nell'era NISQ, approfondisce la Correzione degli Errori Quantistici --- dai codici stabilizzatori (Shor, Steane) al surface code --- inquadrandola come sottocategoria del Quantum Machine Learning, e motiva la scelta di quest'ultimo come approccio adottato in questa tesi.

\textbf{Parte sperimentale (Capitoli 7--14).}

Il \textbf{Capitolo~\ref{chap:metodologia}} (\textit{Metodologia e Architettura}) descrive il pipeline sperimentale: l'architettura del sistema, la struttura dei due metodi sviluppati e le scelte tecnologiche adottate.

Il \textbf{Capitolo~\ref{chap:specifiche}} (\textit{Specifiche Funzionali}) formalizza i requisiti del sistema, i parametri di input e output, la struttura dei quattro use case e le metriche di valutazione.

Il \textbf{Capitolo~\ref{chap:sviluppo}} (\textit{Sviluppo e Implementazione}) presenta il codice sviluppato: il setup dell'ambiente, l'integrazione dell'algoritmo di Shor con il simulatore rumoroso, e l'implementazione concreta dei due metodi.

I \textbf{Capitoli~\ref{chap:risultati1}} e~\textbf{\ref{chap:risultati2}} (\textit{Risultati --- Metodo 1} e \textit{Metodo 2}) riportano i risultati sperimentali sui quattro use case per ciascuno dei due metodi, con analisi statistiche e confronto diretto.

I \textbf{Capitoli~\ref{chap:conclusioni1}} e~\textbf{\ref{chap:conclusioni2}} (\textit{Conclusioni --- Metodo 1} e \textit{Metodo 2}) sintetizzano i risultati di ciascun metodo, ne discutono i limiti e la validità del contributo.

Il \textbf{Capitolo~\ref{chap:sviluppi}} (\textit{Sviluppi Futuri}) delinea le direzioni di ricerca aperte: dal test su hardware reale alla generalizzazione ad altri algoritmi NISQ, dall'evoluzione del classificatore all'integrazione con la QEC strutturale.
