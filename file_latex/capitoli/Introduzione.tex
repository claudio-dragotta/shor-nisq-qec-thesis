...
Il \textbf{Capitolo~\ref{chap:sviluppo}} (\textit{Sviluppo e Implementazione}) presenta il codice sviluppato: il setup dell'ambiente WSL2 con GPU NVIDIA, l'implementazione della QFT, del circuito di Shor, del noise model e delle pipeline di entrambi i metodi.

I \textbf{Capitoli~\ref{chap:risultati1}} e~\textbf{\ref{chap:risultati2}} (\textit{Risultati --- Metodo 1} e \textit{Metodo 2}) riportano i risultati sperimentali sui quattro use case per ciascuno dei due metodi, con istogrammi, analisi statistiche e confronto diretto tramite il fattore $\rho$.

I \textbf{Capitoli~\ref{chap:conclusioni1}} e~\textbf{\ref{chap:conclusioni2}} (\textit{Conclusioni --- Metodo 1} e \textit{Metodo 2}) interpretano i risultati di ciascun metodo, ne discutono i limiti e valutano la validità del contributo ai fini della pubblicazione.

Il \textbf{Capitolo~\ref{chap:sviluppi}} (\textit{Sviluppi Futuri}) delinea cinque direzioni di ricerca aperte: test su hardware reale, generalizzazione ad altri algoritmi NISQ, evoluzione del classificatore, integrazione con la QEC strutturale e scalabilità a istanze di maggiori dimensioni.
