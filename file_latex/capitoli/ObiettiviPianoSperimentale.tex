...
La motivazione crittografica in forma estesa --- il confronto tra la complessità sub-esponenziale del \ac{GNFS} e quella polinomiale $O((\log N)^3)$ di Shor, la conseguente compromissione irrecuperabile di \ac{RSA} indipendentemente dalla lunghezza della chiave, e lo scenario \emph{harvest now, decrypt later} --- è sviluppata nella Sezione~\ref{app:obiettivi} (Sez.~\ref{sec:motivazione}). In sintesi: la sicurezza di RSA cade con la disponibilità di un computer quantistico fault-tolerant, e l'unico ostacolo che oggi impedisce alla minaccia di materializzarsi è il \textbf{rumore} dei dispositivi \ac{NISQ} --- la cui comprensione e mitigazione è la ragione d'essere di questo lavoro.
...
I \textbf{requisiti funzionali} del sistema sperimentale e la specifica completa dei \textbf{parametri di input e output} sono formalizzati nella Sezione~\ref{app:obiettivi} (Sez.~\ref{sec:requisiti} e Sez.~\ref{sec:input_output}): esecuzione configurabile del circuito di Shor, noise model parametrizzabile, raccolta riproducibile degli output e le pipeline dei due metodi. I quattro use case e le metriche di valutazione, che costituiscono il nucleo del contratto sperimentale, sono definiti qui di seguito.
% -----------------------------------------------
...
\begin{itemize}
    ...
    \item \textbf{Asse scalabilità (Use Case 1, 3, 4)}: numeri $N$ di dimensione crescente ($15 \to 21 \to 35$), stesso livello di rumore NISQ-realistico. Il confronto valuta come il numero medio di iterazioni e l'accuratezza del classificatore varino al crescere della profondità del circuito. Si vedrà più avanti (\S\ref{subsec:razionale_istanze}) che l'identificazione fra ``$N$ crescente'' e ``profondità crescente'' non è automatica, e che per Use Case 4 non vale.
\end{itemize}
...
\begin{samepage}
\paragraph{Una precisazione sull'asse di scalabilità.} Le due letture appena date --- Use Case 4 come punto sull'asse $N$ crescente e Use Case 4 come sonda della sola larghezza del registro --- non sono equivalenti, e la verifica sperimentale (Capitolo~\ref{chap:risultati}) mostrerà che la seconda è quella corretta. La profondità del circuito non è governata da $N$ ma dal periodo $r$: ogni gate controllato $U^{2^j}$ con $2^j \equiv 0 \pmod r$ collassa nell'identità e viene eliminato in fase di traspilazione. Con $r=2$ sopravvive un solo gate su dodici, e il circuito di Use Case 4 risulta più corto di quello di Use Case 1 nonostante il modulo maggiore. Lungo la sequenza $N = 15 \to 21 \to 35$ i periodi sono $4 \to 6 \to 2$: la difficoltà non cresce in modo monotòno. L'\textbf{Ipotesi secondaria 2} va quindi intesa nella forma verificabile ``il vantaggio si mantiene al crescere della profondità del circuito'', per la quale l'unica istanza informativa fra quelle previste è Use Case 3.
\end{samepage}
...
Il noise model incorpora il readout error come matrice di confusione simmetrica (parametro $p_{\text{ro}}$) direttamente nel simulatore. Non viene applicata una correzione post-hoc tramite matrice di calibrazione: la robustezza dei metodi al readout error viene invece verificata sperimentalmente nella campagna parametrica (Sezione~\ref{subsec:sweep_secondari} della Sezione~\ref{app:parametrica}), che mostra invarianza di $M_{\text{TOP4}}$ fino a $p_{\text{ro}} = 20\%$.
