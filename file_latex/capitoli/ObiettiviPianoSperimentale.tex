...
\textbf{Confronto sistematico tra approccio statistico classico e classificatore ML.} La maggioranza degli studi di mitigazione del rumore sull'algoritmo di Shor si concentra su tecniche strutturali (Zero-Noise Extrapolation, Probabilistic Error Cancellation, Dynamical Decoupling) applicate al circuito prima della misura. Il presente lavoro adotta invece un approccio \textit{post-processing}: un classificatore binario addestrato sugli output rumorosi del simulatore decide, dopo ogni singola esecuzione, se il risultato è affidabile o meno. Questo paradigma non richiede modifiche al circuito né accesso al modello di rumore dell'hardware, ed è quindi applicabile in modo agnostico rispetto alla piattaforma.

\textbf{Quantificazione empirica del fattore di riduzione $\rho$.} La letteratura fornisce stime teoriche del numero di iterazioni necessarie a ottenere un risultato corretto da Shor in presenza di rumore, ma raramente misure sperimentali sistematiche su più configurazioni. Il presente lavoro si propone di misurare $\bar{M}_1$ e $\bar{M}_2$ su quattro use case distinti --- due livelli di rumore per $N=15$, e due istanze di dimensione crescente ($N=21$, $N=35$) --- producendo una stima empirica di $\rho$ validata statisticamente.

\textbf{Analisi del ruolo dell'entanglement e del tempo di coerenza $T_2$.} Il lavoro collega esplicitamente la degradazione delle prestazioni dell'algoritmo alla perdita di coerenza dell'entanglement tra i registri di controllo e di lavoro, mettendo in relazione i parametri fisici $T_1$ e $T_2$ del simulatore con il numero di iterazioni osservato. Questo collegamento --- spesso assente nelle trattazioni puramente algoritmiche --- fornisce una motivazione fisica al vantaggio del Metodo 2: il classificatore ML impara implicitamente a riconoscere le signature di output associato a circuiti che hanno mantenuto la coerenza sufficiente, senza che questo vincolo sia codificato esplicitamente nelle feature.
