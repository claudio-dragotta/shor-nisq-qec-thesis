...
| M8 Shor logico | ✅ COMPLETO (2026-07-13) | `M8_shor_logico/shor_logico.py`: inietta p_L per gate su Shor N=15, curva P_success vs p_L (metrica per-misura, monotona VERDE: 0.75→0.25). FIGURA CONCLUSIVA `qec_shor_logico_curve`: 3 regimi fisico(p_L~1e-2,P≈0.64)/Steane(~1.7e-3,0.72)/surface d=7(~1e-4,0.74). Sezione+discussione Cap. 13. Barriera: N=21 intrattabile (depth 9500) → si resta a N=15 (conforme doc §2.2). Nota: N=15 degenere (r=4) → TOP-4 non monotono, 80% non raggiungibile per natura → break-even relativo + resource estimates. Compila 170 pp, 0 undefined |
