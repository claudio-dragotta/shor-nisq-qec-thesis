...
calibrazione e scelta della baseline. Le analisi seguenti rendono verificabili
i limiti del decoder ibrido senza interrompere il filo dei risultati principali.
...
I sette esperimenti principali e i controlli successivi E8--E12 convergono
su una conclusione più informativa di un semplice confronto fra decoder.
\textbf{Sostituire} il decoder analitico con una rete non conviene: a
parità di modello di rumore la rete pareggia a distanza 3 --- al prezzo
di $10^5$--$10^6$ campioni --- e perde al crescere della distanza.
\textbf{Affiancarlo} sì: un decoder ibrido in cui la rete impara soltanto
\emph{quando ribaltare} la decisione del matching riduce l'errore logico
del $18$--$21\%$ a distanza 3 e dell'$1$--$3\%$ a distanza 5 in
presenza di rumore correlato, e ricade sul matching quando il rumore è
quello previsto.
...
capacità modesta, scelta per continuità con la prima fase; i decoder allo
stato dell'arte adottano architetture ricorrenti e convoluzionali che
sfruttano località e struttura temporale~\cite{alphaqubit2024}. Le
distanze arrivano a $d=7$, dove il vantaggio si annulla, e il rumore
correlato principale resta di tipo Pauli. Errori coerenti e leakage sono
un asse ulteriore, ma il controllo della Sezione~\ref{sec:oltre_pauli}
non mostra un vantaggio appreso statisticamente risolto specifico per la
sovrarotazione: la proprietà decisiva osservata è la non decomponibilità
del meccanismo in archi, non il generico allontanamento dal modello Pauli.

Resta il fatto metodologico, ripreso nell'analisi di sensibilità di Shor del Capitolo~\ref{chap:risultati}, che collega questa sezione alla prima fase sperimentale: in entrambe le fasi sperimentali di questo lavoro il \textbf{complemento a basso costo ha battuto la sostituzione ambiziosa}. Nella prima, la ricerca multi-candidato TOP-4 ha battuto il classificatore che avrebbe dovuto filtrare l'output; nella seconda, una rete che si limita a segnalare quando il decoder sbaglia batte la rete che pretende di sostituirlo. L'apprendimento automatico non è né inutile né risolutivo: produce valore esattamente dove dispone di informazione che nessun metodo più semplice è in grado di sfruttare.
