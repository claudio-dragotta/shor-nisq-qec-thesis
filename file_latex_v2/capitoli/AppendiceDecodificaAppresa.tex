...
I quattro esperimenti affrontano domande successive. E1 verifica se la rete possa sostituire il matching quando il modello è corretto. E2 aggiunge il crosstalk. E3 controlla se la rete riconosca i fallimenti del matching. E4 usa questa capacità per costruire il decoder ibrido. Trasferibilità, calibrazione, confronto con BP+OSD e limiti di scala sono approfonditi nella Sezione~\ref{app:decodifica} dell'Appendice~\ref{app:fase2}.

La Sezione~\ref{sec:oltre_pauli} ha discusso le approssimazioni del modello usato per \emph{simulare} il codice. Anche il modello usato per \emph{decodificare} ha un limite. Il \ac{MWPM} descrive gli errori mediante un grafo: un arco collega al più due detection events. La \textbf{decomponibilità in archi} rende possibile il matching a peso minimo ed è richiesta dalla costruzione del modello con \texttt{decompose\_errors=True}. Un errore correlato su due qubit dati adiacenti può però attivare fino a quattro detector. Non è quindi rappresentabile direttamente con un solo arco, pur restando un errore Pauli. Il limite del decoder è distinto dall'approssimazione del canale di rumore.

La domanda è se una rete che apprende dai campioni possa superare il decoder analitico quando il grafo non rappresenta alcune correlazioni. Nel Capitolo~\ref{chap:risultati}, il classificatore sull'output di Shor non dà un beneficio rispetto a TOP-4. Qui un percettrone multistrato di \texttt{scikit-learn} riceve invece la \textbf{sindrome}, che contiene relazioni spaziali e temporali assenti dal solo istogramma finale. Il confronto riguarda dunque due usi diversi dell'apprendimento. Non isola il solo effetto del punto di applicazione, perché cambiano anche compito e modello.
...
E1b verifica direttamente l'effetto del numero di dati, mostrato nel pannello (b) della Figura~\ref{fig:qec_neural_leale}. Si fissano $d=3$ e $p = 3\times10^{-3}$ e si varia solo l'insieme di addestramento. Il rapporto cresce da $0.50$ con $2.5\times10^4$ campioni a $1.02$ con $8\times10^5$. Nell'ultimo punto, la rete ha $p_L = 3.98\times10^{-3}$ e il matching $4.04\times10^{-3}$. Il piccolo sorpasso vale però solo $0.2$ deviazioni standard su $10^5$ campioni di test: \textbf{non è statisticamente significativo}. Per pareggiare a distanza 3 servono quindi circa $10^5$--$10^6$ campioni. La crescita osservata mostra che aggiungere dati aiuta in questa configurazione, senza escludere altri limiti del modello. I decoder neurali più avanzati usano architetture ricorrenti e molti più dati~\cite{alphaqubit2024, transformer_decoder2023}. Yan, Li e Du studiano codici fino a distanza $9$ ($161$ qubit), considerando accuratezza e latenza, e attribuiscono un ruolo centrale alla scala dei dati~\cite{yan2026}. Qui si varia il campione a rete fissa; nel loro lavoro si confrontano più architetture.
...
E2 aggiunge un canale correlato fra \textbf{qubit dati adiacenti} sulla griglia, una volta per ciclo di sindrome e con intensità $p_{ct}$. È un modello elementare di \textbf{crosstalk}, discusso nel Capitolo~\ref{chap:rumore}. L'errore fisico di base resta $p = 3\times10^{-3}$, sotto la soglia della Sezione~\ref{sec:surface_risultati}. Si confrontano tre metodi: il matching con rumore uniforme nominale, il matching con un modello che include il crosstalk e la rete addestrata sui campioni del circuito. Le diciture ``nominale'' e ``reale'' nelle tabelle distinguono questi due modelli simulati, non dati raccolti su hardware.
...
Includere il crosstalk nel modello del matching non aiuta sempre. A $d=3$, questo braccio è il \textbf{peggiore} dei tre: a $p_{ct} = 5\times10^{-3}$ dà $5.7\times10^{-2}$, contro $3.1\times10^{-2}$ del modello nominale. Una possibile spiegazione è la scomposizione richiesta dal grafo. La libreria deve rappresentare l'errore correlato tramite più archi; su un grafo piccolo, questa approssimazione può indirizzare il matching verso correzioni sbagliate. A $d=5$ il bilancio cambia e il modello con crosstalk è il migliore. Conoscere il canale non basta quindi a garantirne una rappresentazione adeguata nel decoder.

Il risultato distingue due problemi: stimare correttamente il rumore e rappresentarlo nel decoder. Se il grafo non descrive le correlazioni rilevanti, una calibrazione più precisa può non migliorare la correzione. Nei casi provati può persino peggiorarla.
...
Per $d=3$ e $p_{ct} = 10^{-2}$, il matching sbaglia $5\,966$ shot su $10^5$. Il segnale della rete distingue i fallimenti con \ac{AUC} $= 0.941$. La Tabella~\ref{tab:neural_flag} mostra il compromesso fra precisione e copertura: abbassando la soglia si segnalano meno shot, ma le segnalazioni sono più affidabili. Alla soglia più restrittiva si segnala lo $0.59\%$ delle esecuzioni e l'$85.9\%$ di queste segnalazioni corrisponde a errori veri. Questo non significa riconoscere l'$85.9\%$ di tutti gli errori: la recall è $0.085$.
...
\end{equation}
dove $\oplus$ è la somma modulo 2. La rete riceve la sindrome e la decisione del matching e stima se quest'ultima sia errata. La soglia $\tau$ viene scelta su dati di \textbf{validazione separati} e poi applicata senza modifiche al test. Usare il test per sceglierla renderebbe la verifica dipendente dai dati da valutare. Tra le opzioni c'è anche una soglia che non inverte mai la decisione: se nessuna modifica conviene in validazione, l'ibrido usa il matching puro.

La rete non deve più ricostruire da zero la decodifica. Il matching risolve già la maggior parte dei campioni; la rete cerca soltanto di riconoscere i suoi errori residui. Questo compito più circoscritto permette di ottenere benefici anche dove la sostituzione completa fallisce.
...
La frequenza delle inversioni cresce con il crosstalk: a distanza 3 passa dallo $0.10\%$ all'$11.04\%$. Quando il crosstalk è assente, a $d=5$ non ci sono inversioni e l'ibrido coincide con \ac{MWPM}. A $d=3$ viene invertito lo $0.10\%$ degli shot, senza effetto significativo ($p = 0.67$). Il decoder può quindi tornare al matching quando la validazione non indica un beneficio. Questo criterio non garantisce un miglioramento su ogni punto di test: a $d=3$, $p_{ct}=0$ l'errore è leggermente maggiore, entro l'incertezza statistica. Nelle configurazioni provate, l'ibrido non risulta peggiore del matching in modo statisticamente rilevabile.
