...
L'algoritmo di Shor~\cite{shor1994} fattorizza un intero composto in tempo
polinomiale nella taglia dell'input. La sua complessità è
$O\!\left((\log N)^{3}\right)$; il \emph{general number field
sieve}~\cite{lenstra1993gnfs}, il miglior algoritmo classico noto, richiede
invece tempo sub-esponenziale. È questo divario a minacciare
RSA~\cite{rsa1978}, ma si tratta di
un vantaggio asintotico che i dispositivi \emph{Noisy Intermediate-Scale
Quantum}~\cite{preskill2018} non realizzano. Il rumore distrugge la struttura
periodica su cui l'algoritmo si fonda e le dimostrazioni restano confinate a
moduli didattici~\cite{skosana2021}. Anche la stima delle risorse per RSA-2048,
pur scendendo di circa un ordine di grandezza --- da venti milioni di qubit
fisici nel 2019~\cite{gidney2021} a meno di un milione nel
2025~\cite{gidney2025} ---, resta lontana dai dispositivi disponibili. I due
rimedi principali sono la mitigazione a valle e la correzione d'errore durante
il calcolo; a entrambi un numero crescente di lavori applica modelli appresi,
spesso senza un termine di confronto che separi il contributo del modello da
quello della regola decisionale.
...
hash della netlist). Una demo web separa esecuzione ideale e rumorosa; codice, seed e artefatti
sono pubblici nel repository \url{https://github.com/claudio-dragotta/shor-nisq-qec-thesis} e la
demo è consultabile all'indirizzo \url{https://shor-demo-6knp.onrender.com}.
...
RMS $0{,}08$ in $\ln p_L$; la decodifica appresa, nelle quattordici configurazioni testate ($d=3,5$), non supera
mai il matching come sostituto e, come correttore ibrido, riduce $p_L$ del $18$--$21\%$ a $d=3$ \emph{soltanto in presenza di errori
...
\textbf{Trasformata approssimata.} Una verifica esplorativa condotta a chiusura del lavoro
mostra che troncare le rotazioni della \ac{QFT} sotto una soglia d'angolo dimezza le porte
a due qubit del circuito per $N=21$ (da 49\,661 a 23\,178) e, sulla stima di fase, supera la
trasformata piena in tutti e nove i confronti eseguiti su tre dimensioni di registro, fino a
$+0{,}126$ su dieci qubit di conteggio. Il grado ottimo di troncamento non cresce con il
registro, mentre il risparmio sì.
...
rappresenta. Il criterio unificante è verificabile entro il perimetro misurato: un
modello appreso aggiunge valore quando sfrutta informazione che il metodo di riferimento
lascia inutilizzata, non quando ne replica la regola decisionale. Il braccio negativo dice
che cosa non si ottiene con le risorse di questo lavoro, e non si estende ai decoder
appresi in generale.
...
costo dalle campagne rumorose, l'accelerazione GPU prevista non è stata impiegata e nessun esperimento è stato eseguito su
hardware reale: l'accesso a una macchina è stato valutato e scartato d'accordo con il
relatore, sicché tutta la validazione poggia su modelli di rumore e snapshot di
calibrazione.
...
le porte, perché nel regime \ac{NISQ} il vincolo dominante è la profondità rispetto al tempo
