...
La tesi ha studiato quanta informazione si possa recuperare dall'output rumoroso di Shor e quando serva correggere gli errori durante il calcolo. Su $N=15$, cercare più candidati migliora l'uso dei risultati già misurati. Il classificatore applicato allo stesso output non dà un beneficio aggiuntivo. La seconda fase studia quindi la protezione dello stato e l'uso delle sindromi per scegliere le correzioni.
...
sono corretti globalmente fra otto famiglie. $N=21/35$ non partecipano
alla campagna rumorosa principale; su $N=21$ sono state eseguite soltanto
prove AQFT esplorative a budget ridotto (Sezione~\ref{sec:app_aqft}).
Nella QEC, Steane non ha estrazione fault-tolerant, memoria e proxy per
...
Nei test, un metodo aggiuntivo è utile quando sfrutta informazione trascurata dal riferimento. TOP-4 prova candidati già presenti nell'istogramma. Il decoder ibrido riconosce alcuni errori che il grafo del matching non rappresenta. Aggiungere complessità non basta: occorre misurare un beneficio rispetto alla regola più semplice e al costo richiesto. TOP-K migliora la ricerca sugli esiti misurati, ma non protegge lo stato durante il circuito. Per la scalabilità servono operazioni logiche fault-tolerant; l'apprendimento può assisterne la decodifica quando resta informazione utile che il metodo analitico non usa.

L'AQFT interviene sul circuito prima dell'esecuzione. Su $N=21$, troncare le rotazioni degli addizionatori quasi dimezza le porte a due qubit ($49\,661\rightarrow23\,178$), ma riduce il successo ideale di circa $6.5$ punti. Il possibile margine di tre punti viene dal modello ausiliario a miscela e non è osservato nei test rumorosi. Nel modello richiederebbe errori quasi duecento volte inferiori a quelli della calibrazione considerata; il circuito dura circa 150 volte il tempo di coerenza. La sola stima di fase è molto più corta: qui la variante troncata ha un risultato migliore nei nove confronti e arriva a $+0.126$ su dieci qubit. Il grado migliore resta due o tre alle taglie provate e il risparmio cresce con il registro. Le fasi esatte beneficiano di più, aiutando a spiegare la differenza fra $r=4$ per $N=15$ e $r=6$ per $N=21$. Conteggi e calibrazione suggeriscono i gradi candidati, ma la scelta viene verificata su dati separati da quelli di selezione. Dati, limite teorico e limiti di generalizzazione sono nella Sezione~\ref{sec:qft_topologia}.
