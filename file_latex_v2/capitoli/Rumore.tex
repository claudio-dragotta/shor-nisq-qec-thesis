...
Per l'analisi contano tre vincoli. Le porte sono realizzate con impulsi analogici, che possono essere imprecisi. Il rapporto fra la loro durata e i tempi $T_1,T_2$ limita la profondità utile del circuito. Infine, qubit non adiacenti richiedono operazioni aggiuntive di collegamento, dette routing. La disposizione dei qubit condiziona quindi il costo delle porte a due qubit e il crosstalk. Architettura criogenica, tecnologie, lettura fisica e ``bordo di coerenza'' sono approfonditi nella Sezione~\ref{app:anatomia} dell'Appendice~\ref{app:teoria}.

\section{Le Quattro Fonti Fisiche di Rumore}
\label{sec:fonti_fisiche}

La Tabella~\ref{tab:fonti_rumore} riassume quattro sorgenti di rumore. $T_1$ descrive il decadimento $\ket{1}\rightarrow\ket{0}$; $T_2$ comprende questo rilassamento e la perdita di fase. Gli errori di gate possono derivare da impulsi imprecisi, accoppiamenti residui o leakage, cioè uscita dai livelli usati come qubit. Il readout può scambiare gli esiti della misura. Il crosstalk introduce invece errori correlati fra qubit vicini. Formule, valori tecnologici e matrici di calibrazione sono nella Sezione~\ref{app:fonti_fisiche_estese}.
...
La \ac{QFT} richiede precisione nelle rotazioni di fase, soprattutto in quelle più piccole. Nella \ac{QPE}, il controllo deve conservare la coerenza durante l'esponenziazione modulare. L'aritmetica aggiunge poi molte porte a due qubit, e con esse occasioni di errore. Se si assumono $L$ eventi indipendenti di probabilità $\epsilon$, la probabilità che non ne avvenga nessuno è $(1-\epsilon)^L$. È un indicatore semplificato, non la probabilità di successo di Shor. Derivazione, esempio e Figura~\ref{fig:psuccess_vs_n} sono nella Sezione~\ref{app:shor_rumore_esteso}.
