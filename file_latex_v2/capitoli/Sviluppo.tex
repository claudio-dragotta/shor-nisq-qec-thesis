...
M2 viene addestrato prima della campagna e applicato agli istogrammi durante l'esecuzione. La stessa netlist genera le etichette TOP-1 e TOP-16. Random Forest, \ac{SVM} RBF e \ac{MLP} sono confrontati con una suddivisione stratificata dei dati. Il modello schema~2 salva etichetta, revisione, preset, seed e hash del circuito; l'inferenza rifiuta manifest incompatibili. Quando il filtro accetta l'istogramma, vengono provati fino a quattro candidati. M1, TOP-4 e M2 condividono quindi conteggi, regole per i pareggi e post-processing. I Listati~\ref{lst:generazione_del_dataset_con} e~\ref{lst:loop_di_inferenza_del} mostrano il flusso; l'audit completo è nella Sezione~\ref{app:classificatore}.
...
Nei test QEC si prepara lo stato logico, si inserisce il rumore, si misura la sindrome tramite le ancille e si inviano i segnali al decoder. Si registra poi se la correzione ha avuto successo. Ripetizione, Steane e surface code seguono questo schema con protezioni diverse. L'analisi di Shor è separata: applica un modello di errore per gate e misura la probabilità di ottenere i fattori. Il tasso di fallimento della memoria QEC non viene quindi usato come errore di ciascuna porta di Shor.
